...
object DoobieLogHandler:
  private def createLogHandler(using Logger[IO]): LogHandler[IO] = {
    case Success(s, a, l, e1, e2)              =>
      Logger[IO].debug(s"""Successful Statement Execution:
         |
    ...
    case ProcessingFailure(s, a, l, e1, e2, t) =>
      Logger[IO].error(s"""Failed Resultset Processing:
         |
         |  ${s.linesIterator.dropWhile(_.trim.isEmpty).mkString("\n  ")}
         |
         | arguments = [${a.allParams.flatten.mkString(", ")}]
         ...
         |   failure = ${t.getMessage}
              """.stripMargin)
    case ExecFailure(s, a, l, e1, t)           => Logger[IO].error(s"""Failed Statement Execution:
         |
         |  ${s.linesIterator.dropWhile(_.trim.isEmpty).mkString("\n  ")}
         |
         | arguments = [${a.allParams.flatten.mkString(", ")}]
